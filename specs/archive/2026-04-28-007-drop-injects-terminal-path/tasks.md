...
- [x] 1. Rename drops storage path in repo.ts (`.drops` → `.pier/drops`)
  - agent: main
  - depends: []
  - file_targets: [apps/backend/src/infra/repo.ts]
  - boundary: [apps/backend/src/infra/repo.ts, apps/backend/src/infra/repo.test.ts]

- [x] 2. Add writeChars to TerminalSessions interface and implementations
  - agent: main
  - depends: [1]
...
- [x] 3. Extend projects-drop route to call writeChars and return injected field
  - agent: main
  - depends: [2]
...
- [x] 4. Update frontend drop handler to branch on injected response field
  - agent: main
  - depends: [3]
