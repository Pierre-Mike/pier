...
- [ ] 4. Implement `drops.routes.ts` — POST /api/drops and GET /api/drops (gate file goes GREEN)
  - agent: main
  - depends: [3]
...
- [ ] 6. Delete per-project drop route and remove `saveDropped` from `projects.files.repo.ts`
  - agent: main
  - depends: [5]
  - file_targets: [apps/backend/src/features/projects/projects.drop.routes.ts, apps/backend/src/features/projects/projects.drop.routes.test.ts, apps/backend/src/features/projects/projects.files.repo.ts]
...
- [ ] 7b. [P] Add "Drops" tab and panel to `settings.ts`; wire `GET /api/drops` + copy buttons
  - agent: main
  - depends: [5]
...
- [ ] 8. Regenerate `packages/api-contract/` from updated `AppType` (auto-derived)
  - agent: main
  - depends: [7a, 7b]
